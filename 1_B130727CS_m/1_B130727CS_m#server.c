...
int main()
{
	int sockfd, connectfd, bindfd, listenfd, acceptfd, clientaddresssize, msgsize, i=0, j=0, n, z, temp, arr[10000], choice;
	...
	msgsize = recv(acceptfd, &choice, 4, 0);
	if (msgsize < 0)
	{ 
        printf("Error receiving from socket\n");
	}
	...
	if(choice == 1)
	{
		printf("Sending back nos sorted(asc) : \n");
		for(i=0;i<n;i++)
		{
			z = arr[i];
			printf("Sending %d\n", z);
	...
	{
		printf("Sending back nos sorted(desc) : \n");
		for(i=n-1;i>=0;i--)
		{
			z = arr[i];
			printf("Sending %d\n", z);
